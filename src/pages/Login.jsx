...
import { superAdminLogin } from '../controllers/businessAdmin/auth';
...
const SuperAdminLogin = () => {
  ...
  const handleLogin = async (e) => {
    ...
    e.preventDefault();
    setError('');
    const { success, data, error } = await superAdminLogin(email, password, setLoading);
    ...
    if (success) {
      ...
      setTimeout(() => navigate('/dashboard'), 1500); // Redirect after 3 seconds
    } else {
      setError(error);
  ...
  return (
    ...
    <div className="flex flex-col lg:flex-row min-h-screen bg-secondary">
      {showSuccess && (
        <SuccessStatus user={'Bd Admin'} message={apiResponse?.message} />
        // <SuccessStatus user='Super Admin' />
      )}
      {error && (
      ...
      <div className="lg:flex flex-col justify-center w-full lg:w-1/2 p-16 text-white bg-primary-gradient animate-gradient">
        ...
        <p className="text-lg mb-8 leading-relaxed opacity-90 animate-slide-in">
        ...
        <ul className="space-y-8">
          ...
          <li className="flex items-start gap-4 animate-fade-in delay-150">
            <span className="text-2xl">🔧</span>
            <span>
        ...
        <div className="mt-16 flex gap-8 text-sm animate-fade-in">
      ...
      <div className="flex items-center justify-center w-full lg:w-1/2 p-8 ">
        <div className="max-w-md w-full bg-primary p-10 rounded-3xl shadow-2xl bg-background">
